...
import ru.nabokovsg.documentNK.service.document.protocol.ProtocolControlService;

@RestController
@RequestMapping(
        value = "/document/protocol/control",
        consumes = MediaType.ALL_VALUE,
        produces = MediaType.APPLICATION_JSON_VALUE)
@RequiredArgsConstructor
@Tag(name="Приложения документа",
        description="API для работы с данными приложений документа")
public class ProtocolControlController {

    private final ProtocolControlService service;

    @Operation(summary = "Создать протокол по итогам контроля качества")
    @PostMapping
