...
@Service
public class BoilerRoomWorkJournalClient extends BaseClient {

    private static final String API_PREFIX = "/laboratory/nk/journal/work";
    private static final String DELIMITER = "/";

    @Autowired
    public BoilerRoomWorkJournalClient(@Value("${laboratoryNK-server.url}") String baseUrl) {
        super(WebClient.builder()
                .baseUrl(baseUrl)
                .build());
    }

    public Mono<Object> save(NewBoilerRoomWorkJournalWorkJournalDto journalDto) {
        return post(API_PREFIX, journalDto);
    }

    public Mono<Object> update(UpdateBoilerRoomWorkJournalDto journalDto) {
