...
public class UltrasonicControlClient extends BaseClient {
    ...
    @Autowired
    public UltrasonicControlClient(@Value("${diagnosedNK-server.url}") String baseUrl) {
        super(WebClient.builder()
                .baseUrl(baseUrl)
                .build());
    }

    public Mono<Object> save(NewUltrasonicMeasurementDto measurementDto) {
        return post(API_PREFIX, measurementDto);
    }

    public Mono<Object> update(UpdateUltrasonicMeasurementDto measurementDto) {
